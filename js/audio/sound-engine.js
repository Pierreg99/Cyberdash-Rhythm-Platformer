/**
 * CYBER DASH // CRYO SYNTH AUDIO ENGINE 2.0
 * High-performance Web Audio procedural synthesizer with multi-track sequencer,
 * custom musical scales per level, reactive visualizer, and dynamic SFX.
 */

const SCALES = {
    AEOLIAN: [0, 2, 3, 5, 7, 8, 10],          // Minor - Melodic & driving
    PHRYGIAN: [0, 1, 3, 5, 7, 8, 10],         // Dark Cyber / Industrial
    DORIAN: [0, 2, 3, 5, 7, 9, 10],           // Synthwave / Retro
    PENTATONIC: [0, 3, 5, 7, 10],             // Fast electronic flow
    HARMONIC_MIN: [0, 2, 3, 5, 7, 8, 11]      // High intensity / Boss
};
...
export class CryoAudioEngine {
    ...
    setTrack(config) {
        ...
        this.rootFreq = config.rootFreq || 32.70; // C1 or D1
    ...
    scheduleNote() {
        ...
        // 1. Cyber Kick Drum (beats 0, 4, 8, 12)
        if (beat % 4 === 0) {
        ...
        // 3. Hi-Hat (Every 8th note offbeat)
        if (beat % 2 !== 0) {
            const osc = this.ctx.createOscillator();
            const gain = this.ctx.createGain();
            osc.type = 'square';
    ...
    playSFX(type) {
        ...
        switch (type) {
            ...
            // ── IMMERSIVE MENU BUTTON SOUND FX ─────────────────────────────────
            case 'menu_hover':
                osc.type = 'sine';
            ...
            case 'card_select':
                // Triple-frequency crystalline power hum
